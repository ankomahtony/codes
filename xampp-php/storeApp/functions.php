...
function create_company(){
	...
	$id   = $_POST['cid'];
	$name = e($_POST['com_name']);
	$address = e($_POST['com_address']);
	$tel_number = e($_POST['tel_number']);
	$start_date = $_POST['start_date'];
	$end_date = $_POST['end_date'];

	$sql_company_query		= "SELECT * FROM company WHERE id='$id'";

	$res_sql_user_query	= mysqli_query($connection, $sql_company_query);


	if(mysqli_num_rows($res_sql_user_query) > 0){
		array_push($errors, "Sorry... Id Exists, Choose unique ID");
	}

	$query = "INSERT INTO `company`(`id`, `name`, `address`, `tel_number`, `start_date`, `end_date`) VALUES ('$id','$name','$address','$tel_number','$start_date','$end_date')";
	$results = mysqli_query($connection,$query);

	if ($results) {
		echo $name." has successfully created";
		header("refresh:5; url=index_new.php");
	}else{
		display_error();

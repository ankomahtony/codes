...
$search_result = mysqli_query($connection,"SELECT * FROM `exeat` ORDER BY date_returned DESC");

if(isset($_POST['submit'])){
    $valueToSearch  = $_POST['valueToSearch'];
    $query          = "SELECT * FROM `exeat` WHERE CONCAT(`name_of_student`) LIKE '%".$valueToSearch."%'";
    $search_result  = filterTable($query);
    
    
}else{
    $query          = "SELECT * FROM `exeat`";
    //$search_result  = filterTable($query);
                                    ...
                                    <table class="table table-borderless table-striped table-earning">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-right">Date to Return</th>
                                                <th class="text-right">Issuer</th>
                                        ...
                                        <tbody>
                                            <?php  
                                                ...
                                                $count = 1;
                                    while ($row_exeat = mysqli_fetch_array($search_result)){
                                            if($row_exeat['remark']=="Yes"){
                                                $days_left = 'N/A';
                                            }else{
                                                 $your_date = strtotime($row_exeat['date_of_returning']);
                                                $now = time();
                                                ...
                                                if ($date_diff_round==-0) {
                                                   ...
                                                   $days_left = $date_diff_round." day(s)"; 
                                                }
                                                
                                            }
                                            echo  '<tr>
                                                <td>'.$count.'</td>
                                                <td>'.$row_exeat['name_of_student'].'</td>
                                                <td>'.$row_exeat['current_class'].'</td>
                                                <td>'.$row_exeat['destination'].'</td>
                                                <td>'.$row_exeat['date_of_issue'].'</td>
                                                <td>'.$row_exeat['date_of_returning'].'</td>
                                                <td>'.$row_exeat['issuer'].'</td>
                                                <td>'.$row_exeat['remark'].'</td>
                                                <td>'.$days_left.'</td>
                                                <td>'.$row_exeat['date_returned'].' | '.$row_exeat['time_returned'].'</td>
                                                <td>'.$row_exeat['confirm_by'].'</td>
                                            </tr> ';
                                                $count++;
                                                }
                                            ?>
